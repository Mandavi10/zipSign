...
function ViewHistroy(fileCode) {
    $.ajax({
        ...
        success: function (result) {
            ...
            trailDiv.empty();
            var activityClasses = ["active", "maintxt", "maintxt1", "maintxt2", "maintxt3", "maintxt4"];
            ...
            if (result.responseData.LatestFilePath == null || result.responseData.LatestFilePath == '') {
                ...
                $("#logo").attr("src", "\\Uploads\\" + pathAfterforOriginal);
            }
            else {
            ...
            $("#imgname").html(result.responseData.FileName);
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (result.Status == 1) {
                if (result.msg == 1) {
                    $("#successmsg1").hide();
                    window.location.href = "/SignDocument/DocumentSignRequest?UId=" + UID;
                ...
                if (result.msg == 1) {
                    window.location.href = "/SignDocument/DocumentSignRequest?UId=" + UID;
                    //window.location.href = "/SignDocument/DocumentSignRequest?File=" + result.Path + "&SignerName=" + SignerName + "&Fileid=" + Fileid + "&Emailid=" + Emailid + "&SignerID=" + SignerID + "&UploadedDocumentId=" + UploadedDocumentId;
                } else {
                    $("#successmsg1").empty();
                    var row = '<div class="col-md-12 p-1" role="alert">Please Enter Correct OTP.</div>';
                    $("#successmsg1").append(row);
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (result.UserType == "Single Signer") {
                ...
                var redirectUrl = "/SignDocument/DocumentSignRequest?UType=" + encodeURIComponent(result.UserType) + "&UploadedDocumentId=" + encodeURIComponent(UploadedDocumentId);
